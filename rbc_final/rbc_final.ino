...
int servo_pin[2]={2,3};
int servo_status[4]={};
int motor_pin[4][4]={{8,9,10,11},{15, 14, 13, 12},{4,5,6,7},{16,17,18,19}};
...
//forward系 : 時計回り
//reverse系 : 反時計回り
//dual_ 系 : 向かい合う2つのモータを回転（それぞれ反対方向に)
...
void reverse(int motor_num, int rot){  //1rot = 90 deg  反時計回り。
...
void forward(int motor_num, int rot){
  for(int l=0;l<rot*STEPS/4;l++){
     for(int i=3;i>=0;i--){
       int prev = i+1;
       int next = i-1;
...
void dual_stepper_control(int motor_num, int rot) {
  switch(motor_num){
    case 1:
      if(rot < 0) {
        dual_reverse(1, 3, -rot);
      }
      if(rot > 0) {
        dual_forward(1, 3, rot);
      }
      break;
    case 2:
      if(rot < 0) {
        dual_reverse(4, 2, -rot);
      }
      if(rot > 0) {
        dual_forward(4, 2, rot);
      }
      break;
  }
}
...
void idle_cube(int motor_num, int rot) {
  switch(motor_num){
    case 1: // =(f) 1と3でつかんで、空回りさせる
      cube_hold(1, true);
      cube_hold(3, true);
      cube_hold(2, false);
      cube_hold(4, false);
      //回す 1をrot, 3を-rotだけ回転させる
      dual_stepper_control(1, rot);
      //腕の位置復帰
